  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const STATUS_CONFIG = {
      ...
      rejected: {
        ...
        msg: "O pagamento n\xE3o foi aprovado. Tente novamente para liberar a carta."
      }
    };
    if (!cartaId) {
      showError("ID da carta n\xE3o encontrado na URL.");
      return;
    }
    fetch(`/cartas/${cartaId}`, { headers: { "Accept": "application/json" } }).then((res) => {
      if (!res.ok) throw new Error(`Erro ${res.status} ao buscar a carta.`);
      return res.json();
    }).then((data) => {
      if (!data || data.error) {
        showError(data?.error || "Carta n\xE3o encontrada.");
        return;
      }
    ...
    function renderPorStatus(data) {
      ...
      pendingMsgEl.textContent = config?.msg || "N\xE3o foi poss\xEDvel determinar o status desta carta.";
